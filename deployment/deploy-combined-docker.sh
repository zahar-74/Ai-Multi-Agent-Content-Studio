...
# Deploy combined frontend + backend to Google Cloud Run (LOCAL DOCKER VERSION)
# Faster than Cloud Build when local Docker can reach Artifact Registry.
# Uses local `docker build` + `docker push` instead of `gcloud builds submit`.
...
echo "  ./deployment/deploy-combined-docker.sh"
echo ""
echo "📊 To view logs:"
